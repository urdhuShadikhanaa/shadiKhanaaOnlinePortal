    ...
    compoundFields.forEach((childField) => {
        if (record.fields[childField]) {
            if (isLocalizedFieldType(objectInfo.fields[childField].dataType)) {
                ret.displayValue[childField] = record.fields[childField].displayValue;
            }
...
export function getMissingRelationshipFields(record, relationships) {
    const incompleteFields = Object.keys(relationships).filter((key) => !record.fields[relationships[key].name]);

    return Array.prototype.concat.apply(
        [],
        incompleteFields.map((key) =>
            relationships[key].nameFields.map((nameField) => relationships[key].name + '.' + nameField)
        )
    );
}
...
export function getReferenceRelationships(fields, objectInfo) {
    return fields
        .filter((field) => objectInfo.fields[field] && objectInfo.fields[field].reference)
        .reduce((relationships, field) => {
            const fieldInfo = objectInfo.fields[field];

            relationships[field] = {
                name: fieldInfo.relationshipName,
                nameFields: fieldInfo.referenceToInfos[0] ? fieldInfo.referenceToInfos[0].nameFields : []
            };
...
export const getUiField = (field, record, objectInfo) => {
    ...
    if (fieldInfo.reference) {
        ...
        const fieldData = getCompoundFieldData(field, record, fieldInfo, objectInfo);
...
export const getUiFields = (fields, record, objectInfos) => {
    const fieldValues = fields.map((field) => getUiField(field, record, objectInfos));
...
export function getFieldsForLayout(record, apiName, layoutType) {
    ...
    if (record.layouts) {
        layoutData = extractLayoutFromLayouts(record.layouts, apiName, layoutType || 'Full');
    }
    if (layoutData) {
        const fieldsFromLayout = getFieldsFromLayoutResponse(layoutData, record.objectInfos[apiName]);
...
function extractLayoutFromLayouts(layouts, apiName, layout) {
    ...
    if (layoutId && entityLayout[layoutId] && entityLayout[layoutId][layout] && entityLayout[layoutId][layout].View) {
...
function getFieldsFromLayoutResponse(layout, objectInfo) {
    ...
    const fieldsAccumulator = (listToReduce, fieldsGetterFn, optionalValues) => {
    ...
    const getFieldsFromItem = (item) => {
        return fieldsAccumulator(item.layoutComponents, getFieldsFromLayoutComponent, item.label);
    };
...
export function isPersonNameField(field) {
    return (
        field && (Fields.PERSON_NAME === field.extraTypeInfo || Fields.SWITCHABLE_PERSON_NAME === field.extraTypeInfo)
    );
}