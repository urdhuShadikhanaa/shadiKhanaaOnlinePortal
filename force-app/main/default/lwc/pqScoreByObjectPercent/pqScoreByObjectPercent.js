...
import getHealthScoresForAccountPlan from '@salesforce/apex/HealthScoreController.getIndividualHealthScoresForAccountPlanPercent';

import { formatLabel } from 'c/stringUtils';
import Points_Breakdown_Label from '@salesforce/label/c.Points_Breakdown';

export default class PqScoreByObjectPercent extends LightningElement {
    @api recordId;
        ...
        this.scoreWrappers.forEach((scoreWrapper) => {
            scoreWrapper.config = {
                ...
                data: {
                    labels: [
                        (
                            ((scoreWrapper.points < scoreWrapper.maxPoints
                                ? scoreWrapper.points
                                : scoreWrapper.maxPoints) /
                                scoreWrapper.maxPoints) *
                            100
                        ).toFixed(0) + '%'
                    ],
