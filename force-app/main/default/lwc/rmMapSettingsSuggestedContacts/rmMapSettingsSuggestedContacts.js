...
import updateEnableSuggestedContactsForRMSettings from '@salesforce/apex/RelationshipMapSettingsController.updateEnableSuggestedContactsForRMSettings';
...
import updateRMContactLimit from '@salesforce/apex/RelationshipMapSettingsController.updateRMContactLimit';
...
export default class MapSettingsDefaultValues extends LightningElement {
    ...
    async loadSettings() {
        ...
        await getRMOrgSettingForType({ type: this._settingType })
            .then((result) => {
                ...
                this._showSuggestedContacts = result?.pqcrush__Enable_Sugg_Con_For_RMSettings__c;
    ...
    handleShowSuggestedContactsToggled(event) {
        ...
        this.loading = true;
        updateEnableSuggestedContactsForRMSettings({ settingId: this.settingId, showSuggestedContacts: checked })
            .then(() => {
    ...
    handleUpdateRMorgSettingCondition() {
        updateRMorgSettingCondition({
    ...
    handleSuggestedContactChange(event) {
        updateRMContactLimit({ settingId: this.settingId, contactLimitValue: event.target.value })
            .then(() => {
                this.loading = false;
            })
            .catch((error) => {
